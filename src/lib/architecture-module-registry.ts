import type { LayerSpecItem } from "@/components/ui/LayerSpecAccordion";
...
export type ModuleConfig = {
  ...
  diagram: {
    root: string;
    groups: Array<{ title: string; items: string[]; href?: string }>;
  };
  details: Array<{
    title: string;
    subtitle: string;
...
export const moduleEntries = Object.entries(moduleConfigs) as Array<[ModuleKey, ModuleConfig]>;

export function buildModuleDetailsItems(details: ModuleConfig["details"], moduleSlug: string): LayerSpecItem[] {
  return details.map((item) => ({
    ...
    href: item.href ?? `/docs/architecture/modules/${moduleSlug}`,

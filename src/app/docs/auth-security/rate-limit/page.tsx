...
const details = [
  ...
  },
  {
    ...
    purpose: "After token validation, increment token bucket or sliding window; if over quota, short-circuit with 429 before handler or upstream AI call; otherwise proceed.",
    defines: [
      "Example skew: free tier lower RPM on AI routes; pro tier higher ceiling with separate DSP job caps.",
      "Global emergency throttle can override per-plan ceilings operationally.",
    ],
  ...
  },
  {
    ...
    subtitle: "Limiter decision path",
    purpose: "Concrete ordering after authentication, before expensive handler work (labels only).",
    defines: [
      "1 · Build composite limit key (user / workspace / route family)",
      "2 · Resolve numeric ceiling from plan claim or billing authority",
      "3 · Atomic increment counter in shared store with TTL window",
      "4 · Compare counter to ceiling",
      "5 · Over → attach `429` + backoff metadata; under → `next()`",
    ],
    doesNotDefine: "Downstream provider-side token accounting.",
    href: "/docs/auth-security/rate-limit#runtime-behavior",
    linkLabel: "This section",
  },
  {
    id: "constraints",

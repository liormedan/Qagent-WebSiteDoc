...
const details = [
  ...
  },
  {
    ...
    purpose: "Handler resolves workspace from session → builds query with workspace predicate → returns 404 or empty when ids belong to another workspace (avoid leakage) → writes always include workspace column.",
    defines: [
      "IDs alone never imply access; workspace membership is re-checked.",
      "Background jobs carry workspace id in signed envelopes from API origin.",
    ],
  ...
  },
  {
    ...
    subtitle: "Persistence call path",
    purpose: "Concrete ordering when a server path touches storage (labels only).",
    defines: [
      "1 · Resolve active `workspaceId` from session or signed worker envelope",
      "2 · Open repository / query builder bound to that scope",
      "3 · Append `workspace_id = :ws` (or equivalent) to read predicates",
      "4 · On write: assert row payload workspace matches scope",
      "5 · Execute; apply empty / 404 mapping for foreign ids per policy",
    ],
    doesNotDefine: "Authorization matrix evaluation.",
    href: "/docs/auth-security/data-security#runtime-behavior",
    linkLabel: "This section",
  },
  {
    id: "constraints",

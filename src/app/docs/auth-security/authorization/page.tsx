...
const details = [
  ...
  },
  {
    ...
    purpose: "Handler receives request with req.user from session → resolves workspace from route or body → loads role membership → checks action permission → proceeds or 403.",
    defines: [
      "Resource-level checks where IDs are workspace-scoped (see Data security).",
      "Consistent error shape for forbidden vs not found to avoid enumeration.",
    ],
  ...
  },
  {
    ...
    subtitle: "Policy evaluation order",
    purpose: "Concrete ordering when a protected handler evaluates access (labels only).",
    defines: [
      "1 · Read req.user from decoded session",
      "2 · Resolve target workspace id from route params / body / signed context",
      "3 · Load membership + role for (userId, workspaceId)",
      "4 · Resolve declared action key for this route",
      "5 · Evaluate allow matrix → permit or 403",
      "6 · Continue handler or short-circuit response",
    ],
    doesNotDefine: "QAgent plan content or DSP graph selection.",
    href: "/docs/auth-security/authorization#runtime-behavior",
    linkLabel: "This section",
  },
  {
    id: "constraints",
